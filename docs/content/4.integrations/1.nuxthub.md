...
::note
When both files exist, the NuxtHub schema hook now prefers `schema.<dialect>.mjs` and falls back to `schema.<dialect>.ts`. This avoids Node type-stripping failures when Nuxt resolves `buildDir` to `node_modules/.cache/nuxt/.nuxt` in production builds.
::
